...
export function SettingNumberModal({ roomId, accessToken, onClose, onSubmitted }: SettingNumberModalProps) {
  ...
  const validate = (v: string) => {
    ...
    if (trimmed.length !== 4) return '4자리 숫자를 입력해주세요.';
    const digits = trimmed.split('').map((c) => Number(c));
    const unique = new Set(digits);
    if (unique.size !== 4) return '중복없는 4자리 숫자여야 합니다.';
    return null;
  };
  ...
  const onSubmit = async () => {
    ...
    setError(null);
    setLoading(true);
    const numbers = value.replace(/\D/g, '').split('').map((c) => Number(c));
    try {
