...
export function Button({ children, size = 'md', variant = 'default', ...props }: ButtonInterface) {
  return (
    <button
      {...props}
      className={`whitespace-nowrap cursor-pointer disabled:cursor-not-allowed disabled:bg-gray-200 hover-bg-smooth ${sizes[size]} ${variants[variant]} ${props.className}`}
    >
      {children}
    </button>
  );
